...
<?php
	...
	$result = $mysqli->query("SELECT * FROM biogoal");
	$row = $result->fetch_assoc();
	$totalbottles = $row['totalbottles'];
	$goal = $row['goal'];

	$current = $totalbottles / $goal;
	$current = $current * 100;

	$current = round($current, 2);

?>
<div class="container progress-custom;">
  <div class="progress">
    <div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="<?=$totalbottles?>" aria-valuemin="0" aria-valuemax="<?=$goal?>" style="width:<?=$current?>%">
      <?=$current?>%
    </div>
  </div>
  <p style="text-align: left; font-size: 17px;">Our goal is to collect <?=$goal?> bottles in one month. We are currently at <?=$totalbottles?> bottles collected.</p>
</div>
